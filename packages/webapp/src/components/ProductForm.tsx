...
export default function ProductForm({ product, handleUpdate, handleDelete, handleCreate }: Props) {
  ...
  const formMethods = useForm({ defaultValues: { ...product }, resolver: yupResolver(createProductSchema) });
    ...
    <FormProvider {...formMethods}>
      <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col m-auto bg-white w-1/2 align-middle border-2 rounded-lg p-3 gap-y-4'>
        ...
        <div className='flex justify-evenly gap-2 my-3'>
          ...
          <div className="rounded-lg border p-4 flex flex-col flex-wrap gap-y-8 gap-x-8 text-center basis-1/2" {...register('reheatingInstructions.reheatMode')}>
            ...
            <textarea {...register('reheatingInstructions.reheatInstructions')} className='border p-2 rounded-md' placeholder='Temps et température/puissance' rows={4}></textarea>
          </div>
        </div>
